...
if __name__ == '__main__':
    ...
    # general options
    parser.add_argument('--title',     type=str,            default='few_shot_3dr', help='Title in logs.')
    ...
    parser.add_argument('--basedir',          type=str,            default='/media/jeremy/data/few_shot/data/',       help='Base directory for dataset.')
    ...
    parser.add_argument('--path_to_prep_shapenet',  type=str, default='/media/jeremy/data/prepsn', help='path to prep shapenet')
    ...
    parser.add_argument('--film_type',  type=str, default='dec_3d', help='which part of the network is filmed? options: dec_3d, enc_2d') 
    parser.add_argument('--dec_3d_channels',  type=int, default=1, help='chooses nr of channels') 

    parser.add_argument('--rand_name', type=str, default='') #
    parser.add_argument('--snMini', action='store_true', default=False, help='do we want to use shapenetMini?')
    parser.add_argument('--snMini_size',  type=int, default=50, help='size of ShapeNetMini')
    parser.add_argument('--snMini_mode',  type=str, default='random', help='mode of ShapeNetMini: random,  part, kmeans etc')
    ...
    try:
        ...
        Collector = ShapeNet3DR2N2Collector(base_dir=args.basedir,cat_id = args.cat_id,
            ...
            snMini=args.snMini, snMini_size=args.snMini_size, snMini_mode=args.snMini_mode)
    ...
    # validate that oall data is there
    if 1==2:
        for _ in samples:
            try:
                assert os.path.exists(_[0]) # input image
                assert os.path.exists(_[1]) # gt voxels
            except:
                set_trace()
    #set_trace()
    ...
    try:
        ...
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=args.drop, gamma=0.1)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.rand_name != '':
                filename = '%s_%s_%s_bs_%d_kshot_%s_lr_%g_drop_%d_channels_%d_film_%s_randName_%s_%d.pth.tar'\
                        % (network_name, args.label_type, args.cat_id, args.batchsize, 
                                args.k_shot, args.lr, args.drop, args.dec_3d_channels, args.film_type,args.rand_name, epoch)
            else:
                filename = '%s_%s_%s_bs_%d_kshot_%s_lr_%g_drop_%d_channels_%d_film_%s_%d.pth.tar'\
                        % (network_name, args.label_type, args.cat_id, args.batchsize, 
                                args.k_shot, args.lr, args.drop, args.dec_3d_channels, args.film_type, epoch)
