...
def dirac_delta_torch(phi_pred, eps=0.1):
    '''
    Calculates dirac delta using formula:
    delta(x) = 
        1/(2eps) (1+ cos(pi x / eps)) if |x| <= eps
        0 else
    note:
        if x is a Tensor, then x > 0 will return a 
            ByteTensor with value 1 where x[i] > 0
    '''
...
def my_loss(pred, target, loss_type, p_norm=2, eps=0.1, alpha_sdf=0.1):
    ...
    if loss_type == 'cross':
        ...
        chamf_loss\
            = chamf_loss/torch.max(torch.scalar_tensor(1, dtype=torch.int64),(delta>0).sum())
       
        chamf_loss = chamf_loss**(1/p_norm) # BS, do abs?
        chamf_loss = chamf_loss.sum()
        phi_grad_t = get_grad3d_batch_faster_torch(pred)
        phi_grad_norm_t = get_norm3d_batch_torch(phi_grad_t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--optim', type=str,   default=optim_default, help=('Optimizer [%s]' % ','.join(name2optim.keys()))) # adam or SGD
    parser.add_argument('--lr',    type=float, default=1e-3,          help='Learning rate (default: 1e-3)')
    parser.add_argument('--decay', type=float, default=0,             help='Weight decay for optimizer (default: 0)')
    parser.add_argument('--gamma', type=float, default=0.1,           help='gamma for drop')
    parser.add_argument('--drop',  type=int,   default=30)
    parser.add_argument('--epoch_drop',  type=int,   default=10)
    ...
    parser.add_argument('--eps_delta',  type=float, default=0.1, help='epsilon for dirac delta, default 0.1') 

    parser.add_argument('--k_shot',  type=str, default='full', help='few shot learning. Full means all shot. 0 means just base') 
    parser.add_argument('--freeze',  type=str, default='None', help='freeze weights. Options: None, 2d_encoder, 3d_decoder, all (2d_encoder+3d_decoder)\
            or almost_all (all - linear (bottleneck to 3d decoder), all_3d, all_2d, all_5d')
    parser.add_argument('--embed_init',  type=str, default='None', help='none - random init, avg - average of base classes')
    ...
    parser.add_argument('--k_shot_ver',  type=str, default='False', help='which kshot example are we doing now') 
...
#    set_trace()
    ...
    if 1==1: 
        ...
        train_loader = torch.utils.data.DataLoader(acoto_dataset, \
            batch_size=args.batchsize, shuffle=args.shuffle_val, num_workers=0, \
            pin_memory=True)
...
#    set_trace()
    ...
    try:
        ...
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=args.drop, gamma=args.gamma)
        ...
        print('freeze some weights')
        print('before freezing: ', sum([x.requires_grad for x in net.parameters()]))
        if args.freeze == 'None':
            for c_name, c_param in net.named_parameters():
                if c_name.startswith('model_enc_3d'):
                    c_param.requires_grad = False
        elif args.freeze == '2d_encoder':
            for c_name, c_param in net.named_parameters():
                if c_name.startswith('model_enc')\
                   or c_name.startswith('pre_bottleneck'):
                    c_param.requires_grad = False
        elif args.freeze == '3d_decoder':
            for c_name, c_param in net.named_parameters():
                if c_name.startswith('model_decoder')\
                   or c_name.startswith('model_bottleneck'):
                    c_param.requires_grad = False
        elif args.freeze == 'all':
            for c_name, c_param in net.named_parameters():
                if not c_name.endswith('.bn.embed.weight'):
                    c_param.requires_grad = False
        elif args.freeze == 'all_3d':
            for c_name, c_param in net.named_parameters():
                if not (c_name.endswith('.bn.embed.weight')\
                    and c_name.startswith('model_cBN_decoder')):
                    c_param.requires_grad = False
        elif args.freeze == 'all_2d':
            for c_name, c_param in net.named_parameters():
                if not (c_name.endswith('embed.weight')\
                    and c_name.startswith('model_enc')):
                    c_param.requires_grad = False
        elif args.freeze == 'all_5d':
            for c_name, c_param in net.named_parameters():
                if not (c_name.endswith('embed.weight')\
                    and (c_name.startswith('model_enc')\
                        or c_name.startswith('model_cBN_decoder'))):
                    c_param.requires_grad = False



        elif args.freeze == 'almost_all': # all but model bottleneck
            for c_name, c_param in net.named_parameters():
                if c_name.startswith('model_enc')\
                   or c_name.startswith('pre_bottleneck')\
                   or c_name.startswith('model_decoder'):
                    c_param.requires_grad = False

        print('after freezing: ', sum([x.requires_grad for x in net.parameters()]))
        for uj1, uj2  in net.named_parameters():
            if uj2.requires_grad:
                print(uj1)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.vis_inputs:
                ...
                while True:
                    ...
                    images, labels, cat_nrs, example_ids = coto_dataiter.next()

                    for example_nr in range(args.batchsize):
                        print(example_ids[example_nr])
                        ...
                        if user_response == 'y':
                            ...
                            break 
            tmp_loss_list = []
            for batch_idx, (inputs, targets, cat_nrs, _) in enumerate(train_loader):
                optimizer.zero_grad()
            ...
            network_name = os.path.basename(args.path_to_res)
            filename = '%s_%s_%s_optim_%s_bs_%d_kshot_%s_lr_%g_drop_%d_edrop_%d_gamma_%g_decay_%g_freeze_%s_embInit_%s_channels_%d_film_%s_v_%s_%d.pth.tar'\
                    % (network_name, args.label_type, args.cat_id, args.optim, args.batchsize,
                        args.k_shot, args.lr, args.drop,args.epoch_drop, args.gamma, args.decay,
                        args.freeze, args.embed_init, args.dec_3d_channels,
                        args.film_type, args.k_shot_ver, epoch)


            path_to_out = os.path.join(args.path_to_res, args.cat_id,args.k_shot, args.label_type, 'loss',
                filename+'_loss.txt')
            ...
            if (epoch % 10 == 0 or True):
                ...
                torch.save({'state_dict': net.state_dict(), 
                     ...
                     'optim': args.optim,
                     'lr': args.lr,
                 }, os.path.join(args.path_to_res, args.cat_id, args.k_shot, filename))
    ...
    if not args.no_save or False:
        ...
        torch.save({'state_dict': net.state_dict(), 
                     ...
                     'optim': args.optim,
                     'lr': args.lr,
                 }, os.path.join(args.path_to_res, args.cat_id, args.k_shot, args.label_type, filename)) 
